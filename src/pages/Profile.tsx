            ...
            <TabsContent value="profile">
              <Card className="p-6 bg-card border-none">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {!isEditing && <Button variant="ghost" size="sm" onClick={() => setIsEditing(true)}><Pencil className="w-4 h-4 mr-1" /> Edit</Button>}
                </div>
                {isEditing ? (
                  ...
                  {user?.addresses.length ? user.addresses.map(addr => (
                    <div key={addr.id} className="flex items-start justify-between p-4 bg-background rounded-xl">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{addr.fullName}, {addr.street}, {addr.city}</p>
                      </div>
                  ...
                  {orders.length ? orders.map(order => (
                    <div key={order.id} className="p-4 bg-background rounded-xl">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-xs bg-sage-light text-sage px-2 py-1 rounded-full capitalize">{order.status}</span>
                      </div>
                      <p className="text-sm text-muted-foreground">{order.items.length} item(s) • ${order.total.toFixed(2)}</p>
                    </div>
