              ...
              {[
                ...
                <div key={s.key} className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                    ...
                    ['address', 'payment', 'confirm'].indexOf(step) > i ? 'bg-sage text-primary-foreground' : 'bg-muted text-muted-foreground'
                  }`}>
                    {['address', 'payment', 'confirm'].indexOf(step) > i ? <Check className="w-4 h-4" /> : <s.icon className="w-4 h-4" />}
                  </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
              ...
              {step === 'address' && (
                <Card className="p-6 bg-card border-none">
                      ...
                      {user.addresses.map(addr => (
                        <div key={addr.id} className="flex items-start gap-3 p-4 rounded-xl bg-background">
                          ...
                          <Label htmlFor={addr.id} className="flex-1 cursor-pointer">
                            ...
                            <p className="text-sm text-muted-foreground">{addr.fullName}, {addr.street}, {addr.city}, {addr.state} {addr.zipCode}</p>
                          </Label>
                        </div>
                      ))}
                    </RadioGroup>
                  ...
                  {isAddingAddress ? (
                    ...
                    <Button variant="outline" className="mt-4" onClick={() => setIsAddingAddress(true)}>+ Add New Address</Button>
                  )}
                  <div className="mt-6 flex justify-end">
                    <Button onClick={() => setStep('payment')} disabled={!selectedAddressId}>Continue to Payment</Button>
                  </div>
                </Card>
              )}
                ...
                <Card className="p-6 bg-card border-none">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-background rounded-xl">
                      ...
                      <p className="text-sm">{selectedAddress?.street}, {selectedAddress?.city}</p>
                    </div>
                    {items.map(item => (
            ...
            <Card className="p-6 bg-card border-none h-fit">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="border-t pt-2 flex justify-between font-semibold text-lg"><span>Total</span><span className="text-primary">${total.toFixed(2)}</span></div>
              </div>
            </Card>
          </div>
        </div>
